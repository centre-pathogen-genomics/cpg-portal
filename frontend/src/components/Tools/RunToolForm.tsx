...
const FileParam = ({
  ...
  const [files, setFiles] = useState<
    { label: string; value: string; colorScheme?: string }[]
  >([])
  const types = param.allowed_file_types
    ? param.allowed_file_types.map((t) => t as FileTypeEnum)
    : undefined
      ...
      FilesService.readFiles({
        ...
        for (const file of files.data) {
          flatFiles.push({
            ...
            value: file.id,
            colorScheme: file.is_group
              ? "green"
              : colorSchemeMap[file.file_type ? file.file_type : ""] || "gray",
          })
        }
        return flatFiles
      }),
...
const RunToolForm = ({
  ...
  const onValid: SubmitHandler<any> = async (formData: {
    [key: string]: unknown
  }) => {
    setIsLoading(true)
    ...
    await mutation.mutateAsync({
      body: { params: formData, tags: tags },
      query: {
        tool_id: toolId,
              ...
              <FileParam
                ...
                setValue={(selected) => {
                  if (Array.isArray(selected) || !param.multiple) {
                    ...
                    const existingValues: string[] = getValues(param.name) || []
