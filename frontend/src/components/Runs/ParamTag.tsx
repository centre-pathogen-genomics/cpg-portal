...
const extractUUIDAndOtherText = (value: string | string[]): string => {
  ...
  for (let i = 0; i < values.length; i++) {
    const match = values[i].match(uuidv4Pattern)
    if (match) {
      const uuid = match[0] // Extract the matched UUID
      const otherText = values[i].substring(uuid.length).trim() // Extract the text after the UUID
      text.push(otherText)
    }
  }
  if (text.length === 0) {
...
const ParamTag = ({ param, value }: ParamTagProps) => {
  ...
  return (
    <Tooltip placement="top" hasArrow label={value as string}>
      <Flex
        fontSize={"sm"}
        cursor={"pointer"}
        overflow={"hidden"}
        ...
        borderRadius="md"
      >
        <Flex h={"full"} color={color} bg={bg} px={1}>
          <Text as={"b"}>{param}</Text>
        </Flex>
